...
<template>
  <div class="pm-page">
    ...
    <div class="pm-keys">
      ...
      <button class="pm-reset" @click="reset()">🔄 重置本局</button>
      <button class="pm-start" @click="start()">{{ running ? '⏸ 进行中…' : over ? '🏁 再来一局' : '▶ 开始游戏' }}</button>
    </div>
...
.pm-canvas { width: min(480px, 94%); border-radius: 16px; border: 1px solid rgba(var(--tint-rgb), 0.35); box-shadow: 0 10px 28px rgba(var(--tint-deep-rgb), 0.18); }
.pm-keys {
  display: flex;
  gap: 10px;
  flex-wrap: wrap;
...
.pm-key { width: 48px; height: 42px; border-radius: 12px; font-size: 17px; font-weight: 800; cursor: pointer; background: rgba(var(--panel-rgb), 0.9); border: 1px solid rgba(var(--tint-rgb), 0.4); color: var(--text); }
.pm-reset { padding: 0 24px; height: 42px; border-radius: 12px; font-weight: 700; cursor: pointer; color: #fff; background: linear-gradient(135deg, var(--amber), var(--amber-strong)); border: none; }
.pm-start {
  padding: 12px 34px;
  border-radius: 12px;
  font-weight: 800;
...
.pm-info-mask { position: fixed; inset: 0; z-index: 300; background: rgba(var(--scrim-rgb), 0.45); display: flex; align-items: center; justify-content: center; backdrop-filter: blur(3px); }
.pm-info-box {
...
.pm-mask { position: fixed; inset: 0; z-index: 320; background: rgba(var(--scrim-cool-rgb), 0.5); display: flex; align-items: center; justify-content: center; backdrop-filter: blur(4px); }
.pm-result { width: min(460px, 92vw); max-height: 80vh; overflow: auto; background: rgba(var(--panel-rgb), 0.96); border: 1px solid rgba(var(--tint-rgb), 0.35); border-radius: 18px; padding: 18px 20px; box-shadow: 0 16px 44px rgba(var(--scrim-rgb), 0.4); display: flex; flex-direction: column; gap: 10px; }
...
.pm-again { align-self: center; margin-top: 4px; padding: 10px 28px; border-radius: 999px; font-weight: 800; cursor: pointer; color: #fff; background: linear-gradient(135deg, var(--accent), var(--accent-strong)); border: none; }
