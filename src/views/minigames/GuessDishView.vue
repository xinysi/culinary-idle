...
.gd-mode { padding: 5px 12px; border-radius: 999px; cursor: pointer; font-weight: 700; font-size: 12px; border: 1px dashed rgba(var(--tint-rgb), 0.4); background: rgba(var(--panel-rgb), 0.8); color: var(--muted); }
.gd-mode.on { border-style: solid; border-color: var(--primary-strong); background: rgba(var(--primary-tint-rgb), 0.14); color: var(--primary-strong); }
.gd-chip { padding: 5px 12px; border-radius: 999px; background: rgba(var(--panel-rgb), 0.8); border: 1px solid var(--border); font-size: 12px; font-weight: 700; }
...
.gd-stage { width: min(720px, 98%); min-height: 320px; border-radius: 16px; background: rgba(120, 84, 50, 0.16); border: 1px solid rgba(var(--tint-rgb), 0.35); box-shadow: 0 10px 28px rgba(var(--tint-deep-rgb), 0.18); padding: 26px 18px; display: flex; flex-direction: column; align-items: center; gap: 18px; }
.gd-word { display: flex; gap: 12px; flex-wrap: wrap; justify-content: center; }
.gd-slot { width: 62px; height: 62px; display: flex; align-items: center; justify-content: center; border-radius: 12px; font-size: 30px; font-weight: 800; background: rgba(var(--panel-rgb), 0.9); border: 2px dashed rgba(var(--tint-rgb), 0.45); color: var(--muted); }
.gd-slot.on { border-style: solid; border-color: var(--primary-strong); background: rgba(var(--primary-tint-rgb), 0.12); color: var(--primary-strong); }
...
.gd-char { width: 56px; height: 56px; border-radius: 12px; font-size: 24px; font-weight: 800; cursor: pointer; color: var(--text); background: rgba(var(--panel-rgb), 0.9); border: 1px solid rgba(var(--tint-rgb), 0.4); transition: transform 0.08s, box-shadow 0.1s; }
.gd-char:hover:not(.used) { transform: translateY(-2px); box-shadow: 0 6px 14px rgba(var(--primary-strong-rgb), 0.2); }
.gd-char.used { opacity: 0.3; cursor: default; background: rgba(var(--tint-rgb), 0.12); }
.gd-keys { display: flex; gap: 10px; justify-content: center; align-items: center; flex-wrap: wrap; }
.gd-start { padding: 12px 34px; border-radius: 12px; font-weight: 800; font-size: 15px; cursor: pointer; color: #fff; background: linear-gradient(135deg, var(--accent), var(--accent-strong)); border: none; box-shadow: 0 6px 18px rgba(var(--primary-strong-rgb), 0.35); }
.gd-reset { padding: 10px 24px; height: 46px; border-radius: 12px; font-weight: 700; cursor: pointer; color: #fff; background: linear-gradient(135deg, var(--accent), var(--accent-strong)); border: none; }
.gd-mask { position: fixed; inset: 0; z-index: 320; background: rgba(var(--scrim-cool-rgb), 0.5); display: flex; align-items: center; justify-content: center; backdrop-filter: blur(4px); }
.gd-result { width: min(460px, 92vw); max-height: 80vh; overflow: auto; background: rgba(var(--panel-rgb), 0.96); border: 1px solid rgba(var(--tint-rgb), 0.35); border-radius: 18px; padding: 18px 20px; box-shadow: 0 16px 44px rgba(var(--scrim-rgb), 0.4); display: flex; flex-direction: column; gap: 10px; }
...
.gd-again { align-self: center; margin-top: 4px; padding: 10px 28px; border-radius: 999px; font-weight: 800; cursor: pointer; color: #fff; background: linear-gradient(135deg, var(--accent), var(--accent-strong)); border: none; }
...
.gd-info-mask { position: fixed; inset: 0; z-index: 300; background: rgba(var(--scrim-rgb), 0.45); display: flex; align-items: center; justify-content: center; backdrop-filter: blur(3px); }
.gd-info-box { width: min(620px, 92vw); max-height: 76vh; overflow: auto; background: rgba(var(--panel-rgb), 0.94); border: 1px solid rgba(var(--tint-rgb), 0.35); border-radius: 16px; padding: 16px 18px; backdrop-filter: blur(12px); box-shadow: 0 14px 40px rgba(var(--deep-soft-rgb), 0.35); }
.gd-info-head { display: flex; justify-content: space-between; align-items: center; margin-bottom: 10px; font-size: 15px; }
.gd-info-close { cursor: pointer; border: none; background: rgba(var(--tint-rgb), 0.15); border-radius: 999px; width: 30px; height: 30px; font-weight: 700; color: var(--text); }
.gd-info-list { display: flex; flex-direction: column; gap: 8px; }
.gd-info-row { display: flex; align-items: baseline; gap: 10px; background: rgba(var(--panel-soft-rgb), 0.8); border: 1px solid rgba(var(--tint-rgb), 0.2); border-radius: 10px; padding: 8px 12px; }
