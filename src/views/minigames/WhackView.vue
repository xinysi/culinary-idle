...
function settle() {
  if (overFlag) return
  player.recordMinigame('whack', score.value, { lower: false, unit: '分' })
  overFlag = true
  over.value = true
  running = false
  stopLoop()
      ...
      <div class="wk-info-box">
        ...
        <div class="wk-info-list">
          ...
          <div v-for="(m, key) in MODES" :key="key" class="wk-info-row">
            ...
            <span class="wk-info-desc" v-html="m.desc"></span>
          </div>
          <div class="wk-info-row wk-info-rule">
...
.wk-canvas { width: min(1000px, 98%); border-radius: 16px; border: 1px solid rgba(var(--tint-rgb), 0.35); box-shadow: 0 10px 28px rgba(var(--tint-deep-rgb), 0.18); cursor: pointer; touch-action: none; }
.wk-keys {
  display: flex;
  gap: 10px;
...
.wk-reset { padding: 10px 24px; border-radius: 12px; font-weight: 700; cursor: pointer; color: #fff; background: linear-gradient(135deg, var(--accent), var(--accent-strong)); border: none; }

.wk-mask { position: fixed; inset: 0; z-index: 320; background: rgba(var(--scrim-cool-rgb), 0.5); display: flex; align-items: center; justify-content: center; backdrop-filter: blur(4px); }
.wk-result { width: min(460px, 92vw); max-height: 80vh; overflow: auto; background: rgba(var(--panel-rgb), 0.96); border: 1px solid rgba(var(--tint-rgb), 0.35); border-radius: 18px; padding: 18px 20px; box-shadow: 0 16px 44px rgba(var(--scrim-rgb), 0.4); display: flex; flex-direction: column; gap: 10px; }
...
.wk-again { align-self: center; margin-top: 4px; padding: 10px 28px; border-radius: 999px; font-weight: 800; cursor: pointer; color: #fff; background: linear-gradient(135deg, var(--accent), var(--accent-strong)); border: none; }
...
.wk-info-mask { position: fixed; inset: 0; z-index: 300; background: rgba(var(--scrim-rgb), 0.45); display: flex; align-items: center; justify-content: center; backdrop-filter: blur(3px); }
.wk-info-box {
