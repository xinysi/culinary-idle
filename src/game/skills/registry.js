...
import { PotterySkill, WeavingSkill, EmbroiderySkill, CandleMakingSkill, FletchingSkill, NetmakingSkill, IncenseSkill, FestivalGoodsSkill, JadecraftSkill, GoodsTagSkill, MiningGearSkill, PapermakingSkill, InstrumentSkill, SoapmakingSkill, ExchequerSkill } from './SidelineSkills.js'
...
const factories = {
  ...
  fletching: (player) => new FletchingSkill(player),
  netmaking: (player) => new NetmakingSkill(player),
  ...
  // v2.14.0 四支
  papermaking: (player) => new PapermakingSkill(player),
  instrument: (player) => new InstrumentSkill(player),
  soapmaking: (player) => new SoapmakingSkill(player),
  exchequer: (player) => new ExchequerSkill(player),
  // §3.3.6 食灵召唤 / §3.4 辅助类
