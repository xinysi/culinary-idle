...
/**
 * 击杀后的**重生间隔**（秒）—— 2026-09-22 用户「abc 都做」→ (b)。
 * 参考作 Melvor Idle 有约 3 秒重生间隔，作用不是「拖时间」而是**精准惩罚一击秒杀**：
 * 固定开销摊在越短的战斗上占比越大（2 回合/4 秒的战斗 +1.5s = +38%，40 秒的深塔战 +4%）。
 * ⚠️ 它是**每次击杀**的开销 ⇒ 会按比例压低「每小时击杀数」（进而 XP/金币/掉落时产）：
 *   实测目标是把同等级单场从 4~11s 拉到 8~12s，重生 1.5s 摊上去后总节奏 ≈ 10~14s/杀，
 *   与 Melvor 的「数秒~数十秒」同带（ 复测）。
 * 🔴 只作用于**胜利**：输掉不重生（怪物还在原地），所以「挑战失败」不会额外被罚时间。
 */
export const COMBAT_RESPAWN_SEC = 1.5
