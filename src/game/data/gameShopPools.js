 ...
 */
export const DELUXE_EXCLUDED_CATEGORIES = ['buff', 'supply', ...SIDELINE_ITEM_CATEGORIES]
...
export const INGREDIENT_POOL = Object.values(ITEMS)
  ...
  // v2.9.0/v2.10.0：同样排除全部副业类别（木器/陶器/织物/绣品/蜡烛）——它们是各副业的独占产物，
  // 礼包若能开出来就等于绕过整条技能线
  .filter((i) => i.type === 'ingredient' && !RARE_POOL.includes(i.id) && i.category !== 'mineral' && i.category !== 'material' && !SIDELINE_ITEM_CATEGORIES.includes(i.category))
  .map((i) => i.id)
