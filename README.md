...
| 小游戏 | 2026-09 | 🎮 小游戏大厅（顶部导航第三页单入口）：**🛒 游戏商店 + 七款游戏**统一「多模式分类 + 📖模式说明弹窗」；**2026-09-07 货币改革**：七款小游戏奖励全部改为**游戏币**（`player.gameCoins`，像素金币图标 `images/icon-coin.png`，与主金币分离），**游戏商店**（`GameShopView`，大厅第一入口）19 件商品全部真实生效（背包满自动直发仓库）：金币兑换包 100/900/9000 币（+1000/+10000/+100000 金）、稀有食材盲盒 120/1000/10000 币（1~3/10~30/100~300 份）、鲜味食材礼包 500（30~80 份）/珍馐料理礼包 1500（10~20 份）/精酿调料礼包 400（20~50 份）/锻造矿材礼包 800（20~50 份）、双倍挂机药剂(30min)、神秘种子袋 60/500/5000 币（3/30/300 颗）、觅珍抽卡券 200/1800/18000 币（×1/×10/×100，抽卡优先抵扣）、制作加速器(队列快进10分钟)、「大胃王」称号、「神厨」头像框(金色描边)、限定头像·食神盛宴（后三者一次性、购后锁定）· 按钮置底全宽；火候炉（**十模式**：新手宽区/慢火/经典/疾速/快燃/极限单轨 + 双锅同调/疾速/极限 + 精准单轨，速度·双轨·完美区宽参数化）、美食讲堂（**十模式**：知识入门/标准/大师、对比试炼/挑战、连线试炼/挑战、综合标准12题≥10/进阶15题≥13/大师18题≥16，达标得徽章+游戏币）、厨心 2048（**十模式**：3×3/4×4/5×5/6×6 × 出4率 5%/10%/30%/55%，标准合并/每步1新块/单次合并不级联，动画、图片制档位（8192+至高 262144 佛跳墙）、奖励×模式倍率、最佳分按模式独立）、大胃王（**十模式**：15s→300s 时长梯度，暴食×2、提前结算、累计游戏币无上限）、每日美食拼图（**十模式**：2×2~8×8 + 3/4/5×5 限步，图片与乱序每次随机、限步超步失败）、食材连连看（**十模式**：偶数棋盘 2×2/4×4/6×6/8×8/10×10 恰好填满无缺格 + 限时 60/90/120s、限步 16/30 步玩法变体，全清得游戏币）、**食材翻牌**（🧠 独立第七款：牌面 ❓ 记忆配对、3D 翻转/翻错抖动/消除缩放动画、翻错盖回，**十模式**：2×2/4×4/6×6/8×8/10×10 偶数棋盘 × 普通/限时/限翻，最佳=最少翻牌次数）；**2026-09-07 统一**：奖励全部游戏币（无 buff/饼干）、取消每日限制、按钮/文字/状态胶囊统一规范、图鉴物品图片固定尺寸（object-fit contain）；**2026-09-08 凑十消**（🧮 第 13 款）：回字多层嵌套（4×4/6×6/8×8 = 2/3/4 层），外环清空解锁内层（锁定方块禁点无反馈）· 选两个数字和=10 消除（1+9/2+8/3+7/4+6/5+5，5 只对 5）· 十模式（分层数×五比例×步数容错×40%多五/无五/紧凑变体，全清通关 50~360 币）· 层内自足配对保证开局可解 + step 步数与死局双失败判定 · 提示×2/重排×1 道具（Web Audio 音效、消除/解锁/礼花动画、🚪退出回大厅）；**2026-09-08 体验打磨**：①贪吃蛇/吃豆人进入空白修复（onMounted 绘制）；②吃豆人速度放缓（基准 210ms/格、档次 1.0/0.85/0.7）+ rAF 插值平滑；③消消乐重做：短名（新手20/标准20…/地狱15）、tile 绝对定位动画（交换/消除弹出/下落滑入）、高分目标 600~3800（计分 n连×15×1.5^连锁）、选中按 tile 对象修复消除后错位；④笨鸟先飞打磨：雪山像素背景（视差滚动+深色压暗）、上下管道换用户素材（png）并 alpha 裁剪贴边（上贴顶/下贴底）、管道加宽 130px、小鸟 GIF 红雀+8 只像素鸟可选（选鸟行、持久化 minigames.flappy.bird、居中+X 缩放换算）；⑤商店购买按钮橙色、进度条轨道橙金、2048 方向键深色适配；**2026-09-08 笨鸟先飞**：🐦 重力扑翼穿管（Canvas·点画面/空格/↑ 扑翼），十模式（速度×缝宽×重力：悠闲/标准/快速/极速/窄缝×2/重鸟×2/密管/地狱），每穿 1 管 × 单价 1.2~6.5 币结算；**2026-09-08 三款新小游戏**：🐍贪吃蛇（Canvas·速度×边界×障碍 十模式·按食物×模式单价结算）、👻吃豆人（迷宫+幽灵 AI+能量豆减速·幽灵数0~3×速度 十模式·通关结算）、🍬消消乐（8×8 三消+连锁加成·步数型/限时型 十模式·达标结算），全部十模式+统一排版+说明弹窗+深色适配；**2026-09-08 深色适配**：八款小游戏界面全面适配深色模式（html[data-theme="dark"] 覆盖层 100+ 规则），入口改短名（🛒商店/📚讲堂/🧩2048/🧩拼图/🀄连连看/🧠翻牌），奖励与商店定价平衡（大胃王碗×0.33 权重、2048 档位提档、讲堂徽章兑换 200/450/800、连连看下调、商店批量折扣 9000/4500/880/8500/17000、购买按钮橙色、进度条轨道橙金、2048 方向键深色适配） |
