...
| 小游戏 | 2026-09 | 🎮 小游戏大厅（顶部导航第三页单入口）：六款游戏——火候炉（**三玩法**：经典单轨/双锅同调/快燃挑战）、美食讲堂（**三题型**：知识/火眼金睛对比/大厨连线，12 题 ≥10 得徽章）、厨心 2048（**十种模式**：3×3/4×4/5×5/6×6 × 出4率 5%/10%/30%/55%，玩法=标准 2048 合并/每步1新块/单次合并不级联，**动画**（滑动过渡+合并弹出）、图片制档位、奖励金币×模式倍率、最佳分按模式独立）、大胃王（三档时长）、每日美食拼图（**图片拼图**：完整图+同图切块，3×3/4×4/5×5）、食材连连看（图片制，4×4/6×6/8×8）；**2026-09-07 统一**：奖励全部金币（无 buff/饼干）、取消每日限制、按钮/文字/状态胶囊统一规范、图鉴物品图片固定尺寸（object-fit contain） |
