...
FAM = {
    ...
    "页签": dict(
        ...
        subject="横向长方形木牌页签，四角 3px 圆角，未选中状态，无文字",
        slice=10, radius=3, border=1),
    "卡片": dict(
    ...
    "复选框": dict(
        ...
        subject="14×14 正方形小木牌复选框，四角 3px 圆角，未勾选状态，无符号",
        slice=8, radius=3, border=1),
    "表格": dict(
...
for i, ((fam, cls), a) in enumerate(sorted(V.items(), key=lambda kv: (-len(kv[1]["pages"]), kv[0][0], kv[0][1])), 1):
    ...
    states = " / ".join([s for s, _ in a["state"].most_common(4)]) or "常态"
    slice_ = F["slice"]
    rows4.append([
        i, fam, cls, len(a["pages"]), a["n"],
        f"{w}×{h}", rng, r0, f0, p0, f"{r0} → {F['radius']}px",
        f"{w*2}×{h*2}", slice_, f"{slice_}/{slice_}/{slice_}/{slice_}",
        "常态 / 悬停 / 按下 / 禁用" + (" / 选中" if fam in ("页签", "按钮", "主按钮") else "") + (" / 锁定" if "lock" in cls or "锁定" in states else ""),
        F["recipe"],
...
sheet("控件族规范", "控件族规范 · 每个变体的专属尺寸与素材提示词（226 个变体）",
      ["#", "族", "变体类名", "出现页数", "实例数", "实测尺寸（常见）", "实测尺寸范围", "现圆角", "现字号", "现内边距",
       "圆角 → 新", "导出尺寸 @2x", "9-slice 边距(px)", "slice 四边", "状态图清单", "材质配方（四层叠加）",
       "提示词·① 主体+尺寸", "提示词·② 材质", "提示词·③ 风格与光照", "提示词·④ 构图与输出",
