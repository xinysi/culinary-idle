...
// ── 4. 克制三角完整性：每个风格恰好克制一个、被一个克制 ──
{
  ...
  check('对决', '攻略克制指引与数据一致（火锅真君=调味，用摆盘）', STYLE_INFO[COMBAT_BOSSES.find((b) => b.name === '火锅真君').style].name === '调味', 'flavor 应克?')
...
// ── 6. 攻略物品名抽查（全部存在于物品库）──
{
  ...
  const probe = ['烤土豆', '白面包', '能量饼干', '苹果汁', '食盐', '椒盐', '酱油', '神秘调料', '玉米饼', '开水白菜', '佛跳墙', '铜刀', '金刀', '水晶刀', '陷阱', '盛夏草帽', '苹果', '面粉', '茶饮', '保鲜剂', '松露', '灵果']
...
// ── 9. 成就阈值与真实内容同步 ──
{
  ...
  check('成就', '图鉴成就阈值 ≤100 且 collectionPct 上限 100', ['log25', 'log50', 'log75', 'log100'].every((id) => ALL_ACHIEVEMENTS.find((a) => a.id === id)?.check({ collected: Object.fromEntries(Object.keys(ITEMS).map((k) => [k, 1])), seasons: {} }) === true))
}
