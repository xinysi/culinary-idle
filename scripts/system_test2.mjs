...
console.log('══ W5. 竞技场奖励 ══')
{
  ...
  let r = null
  for (let i = 1; i <= 5; i++) r = p.onArenaEnd(true, '镜像A', 30)
  check('奖励', '5 连胜宝箱（+100 金 + 神秘调料）', r?.kind === 'streak' && r.gold === 100 && (p.inventory.mysterySpice ?? 0) === 1 && p.gold === gold0 + 100, JSON.stringify(r))
  // 3 连胜破纪录（新档）：200 金 + 能量饼干
  const p2 = freshPlayer()
  const g2 = p2.gold
  let r2 = null
  for (let i = 1; i <= 3; i++) r2 = p2.onArenaEnd(true, '镜像B', 25)
  ...
  let r3 = null
  for (let i = 1; i <= 10; i++) r3 = p3.onArenaEnd(true, '镜像C', 40)
  check('奖励', '10 连胜宝箱含能量饼干', r3?.kind === 'streak' && r3.gold === 200 && (p3.inventory.energyBiscuit ?? 0) >= 1, JSON.stringify(r3))
  // 战败重置连胜且无奖励
...
console.log('══ W6. 新功能 ══')
{
  ...
  check('签到', '第 1 天签到 +100 金', r1.ok && r1.day === 1 && r1.reward.gold === 100 && p.gold === 200, JSON.stringify(r1))
...
console.log('══ AA. 全域数值安全 ══')
{
  ...
  check('安全', '签到奖励引用物品存在', ['mysterySpice', 'energyBiscuit', 'xpTonic'].every((id) => !!ITEMS[id]))
