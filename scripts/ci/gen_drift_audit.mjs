...
const KNOWN_DRIFT = {
  ...
  // ── 2026-09-25 新增：餐厅装潢的手工函数（decorPerK / nextDecorPick：性价比口径与「下一件推荐」
  //    唯一出口，2026-09-21 用户要求控制小数位）直接写在产物文件末尾；生成器只产数据表，
  //    重跑会删掉这两个函数 ⇒ 不重跑，以仓库版本为准。 ──
  'restaurantDecor.js': '仓库版末尾含手工补写的两个函数（decorPerK/nextDecorPick），生成器只产数据表。不重跑：重跑会删掉这两个函数（装潢页排序与餐厅页提示共用）',
}
