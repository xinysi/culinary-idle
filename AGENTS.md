...
- **播放引擎**（`game/core/sound.js`）：HTMLAudioElement 池 + `loop` + **1.2 秒交叉淡入淡出**；**文件缺失/解码失败会标记该曲不可用并回落到合成音序器**（`TRACKS` 的 day/night/battle 保留至今，专当兜底——打包漏文件时游戏仍有声音）。
  - 🔴 **淡变定时器必须按元素存**（`Map<el, timer>`）：2026-09-17 用户报「点其它音乐时原本的不中断、会重叠播放」——根因就是原先**一个全局 `realFadeTimer`**：「旧曲淡出」与「新曲淡入」两次调用共用它，后者的 `clearInterval` 把前者打断 → 旧曲的 `pause` 回调永不执行、以原音量继续播。不变量：**任意时刻只能有一首在出声**（`bgm.playingCount()` 供守卫断言）。
  改音量会打断淡变 ⇒ `setBgmVolume` 必须顺手把「不是当前曲目」的元素停掉复位，否则会留下「淡到一半的旧曲」。
- **暂停/继续**：`bgm.pause()`（淡出、**保留进度**）/`bgm.isPaused()`/`position()`；暂停状态存 `settings.bgmPaused`（**持久化**，与 `bgmEnabled` 是两回事），由 `syncBgm()` 判定 —— 暂停时只更新「该放哪首」不出声，继续时**从原进度接着放**。
  UI 是胶囊左侧的独立播放/暂停钮（`.bgm-pp`）+ 面板头部同款钮；⚠️ 真实音频的启停有 1.2s 淡变，**断言别按「立即停」写**（`e2e-audio-skin` 曾按旧合成版的 200ms 写、被真实音频打回）。
