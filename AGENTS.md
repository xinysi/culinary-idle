...
- 测试环境：`@playwright/test`（chromium 已装），dev server 在 `http://localhost:5173/` 或 `5174/`（可能有两个实例）；三套 e2e：`e2e-test.spec.mjs`（流程）+ `e2e-dark.spec.mjs`（深色体检）+ `e2e-text.spec.mjs`（前端标记裸露扫描，见下方「富文本必须 v-html」）。
