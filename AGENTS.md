...
- **制作页配方卡要显示精通**（v2.6.1 起）：`经验（含精通倍数高亮 mastery-hl）+ 精通 Lv/100 + 进度条 + x / y 次 · 再 N 次到 M 级`，与采集卡同构；**厨房笔记卡片**另有「下一档 N 级（再 X 次）：经验 ×a→×b、双倍 c%→d%」（只列真的会变的项）。制作类与采集类吃的是**同一组**函数（保底＝额外多出一份产物，双倍＝一次做两份）。
