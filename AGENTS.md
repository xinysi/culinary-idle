...
### 🖼 物品图入库（用户给原图 → 我处理成 64×64）——工具与四个坑（v2.9.1 立）
**工具 = `scripts/gen/process_item_images.py`**（不是数据生成器，不产出 js、不进 `gen_drift_audit`）：
`--stats` 看连通域统计 · `--dry-run` 不写盘 · `--sheet out.png --resample both` 出「近邻 vs 平滑」对照图 · `--core-t` 调阈值。
顺序 = **按文件修改时间升序**一一对应 `DEFAULT_MAP`（用户把图丢进一个文件夹、按时间顺序放，名字要我们改）；源图张数与映射表不等时**直接中止、不写任何文件**。

**源图的四个坑**（两批 AI 图实测完全一致，别每次都重新踩）：
...
4. **物体外圈有 1~2px 抗锯齿白边**。2048→64 是 **32 倍**降采样，用**最近邻**会把白边零星采成「白点」散在物体周围——**用户两批都报过这个**（「图鉴里还能看到白色的东西」）。

**关键判断：②③ 都比「近白」暗，所以不会被当成背景；而「放宽近白阈值」是错的做法**——浅色主体（木长桌、木餐盘的受光面）会跟着一起被吃掉。
**正确做法（已实现）**：按**亮度找主体核心**（`min(R,G,B) <= 185`）+ **只留最大连通域** + **补洞** + **膨胀 4px 收回抗锯齿边**。判据：主体核心亮度中位只有 **17~62**，而水印与投影都在 **200 以上**；阈值在 150~200 之间最大连通域面积几乎不变（21~45% 画布）⇒ 185 很稳。
**降采样默认用 `box`（预乘 alpha 的面积平均）**：二值 alpha 经面积平均后边缘自然得到 1px 抗锯齿，**比最近邻干净得多**（同源的 20 档木材是最近邻、边缘更硬；要完全对齐用 `--resample nearest`）。

**入库后的校验（照做）**：10 张全部 **64×64 RGBA**（A 范围必须含 0 与 255）· **两两不同**（MD5）· **不等于占位图** · 跑 `item_triple_audit`（「全部 N 件物品均有图片」）· 页面实测卡片图标 `naturalWidth` 为 64、破图 0、控制台错误 0。
