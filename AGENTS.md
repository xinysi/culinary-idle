...
- **统计页排版失衡**：6 个区块用固定 `grid` 排 6 列 → 前三列 1812px、后三列 636px，右下大片空白。改为 **CSS 多列（`column-count`，按内容自动配平）+ `break-inside: avoid` 防卡片被切断**；顶部亮点卡由「5+1 落单」改成整行均分。**判据是各列像素高度差，不是目测**。
- **设置面板四种控件观感不一**：复选框/滑块/输入框/下拉各自一套默认样式 → 统一为一套「玻璃框」（滑块轨道与滑钮重做、复选框走主色、输入框与下拉共用边框/内底/圆角）。改设置面板时**只动版式、别改绑定**（`v-model` 与 `id` 一律不动），改完必须跑 `e2e-audio-skin`（它要先点「画面」页签才能操作 `.skin-chip`）。
